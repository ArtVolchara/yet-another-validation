...
describe('isObject validator test', () => {
  describe('Primitive values', () => {
    ...
    primitiveTestCases.forEach(({ input, description }) => {
      test(`Should return error result for ${description}`, () => {
        // Arrange
        const inputValue = input as unknown as Record<string, unknown>;
        const expectedResult = new ErrorResult(IS_OBJECT_ERROR_MESSAGE, undefined);

        // Act
        const actualResult = isObject(inputValue);
  ...
  describe('Non-primitive values', () => {
    ...
    const successTestCases = [
      ...
      { input: { a: 1, b: '2' }, description: 'object with properties' },
      { input: { [Symbol('test')]: 'value' }, description: 'object with symbol keys' },
      { input: { a: 1 }, description: 'object with number property' },
      { input: { a: 'b' }, description: 'object with string property' },
      { input: { a: true }, description: 'object with boolean property' },
      { input: { a: {} }, description: 'object with object property' },
      { input: { a: [] }, description: 'object with array property' },
    ];
