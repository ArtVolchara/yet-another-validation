...
describe('isArrayBuffer validator test', () => {
  describe('Primitive values', () => {
    ...
    primitiveTestCases.forEach(({ input, description }) => {
      test(`Should return error result for ${description}`, () => {
        // Arrange
        const inputValue = input as unknown as ArrayBuffer;
        const expectedResult = new ErrorResult(IS_ARRAY_BUFFER_ERROR_MESSAGE, undefined);

        // Act
        const actualResult = isArrayBuffer(inputValue);
  ...
  describe('Non-primitive values', () => {
    ...
    const successTestCases = [
      { input: new ArrayBuffer(8), description: 'ArrayBuffer with 8 bytes' },
      { input: new ArrayBuffer(16), description: 'ArrayBuffer with 16 bytes' },
      { input: new ArrayBuffer(32), description: 'ArrayBuffer with 32 bytes' },
    ];
