...
export const generateArrayMinLengthErrorMessage = <MinLength extends number>(minLength: MinLength) => `Array should contain more than ${minLength} elements` as const;
export type TIsArrayMinLengthValidationError<MinLength extends number> = IError<ReturnType<typeof generateArrayMinLengthErrorMessage<MinLength>>, undefined>;
...
export default function generateArrayMinLengthValidationRule<MinLength extends number>(
  minLength: MinLength,
  ...
  function isArrayMinLength<const Params extends TValidationParams | undefined = undefined>(
    ...
    if (params?.shouldReturnError === true) {
      return new ErrorResult(generateArrayMinLengthErrorMessage<MinLength>(minLength), undefined) as TIsArrayMinLengthValidationResult<MinLength, Params>;
    }
    try {
